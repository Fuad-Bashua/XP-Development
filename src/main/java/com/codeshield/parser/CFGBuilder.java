...
public class CFGBuilder {
    ...
    public List<ControlFlowGraph> buildAll(List<String> lines) {
        ...
        for (FunctionBlock func : extractFunctions(lines)) {
            cfgs.add(buildCFG(func));
        }
    ...
    private List<FunctionBlock> extractFunctions(List<String> lines) {
        ...
        for (int i = 0; i < lines.size(); i++) {
            ...
            functions.add(new FunctionBlock(funcName, startLine, bodyLines, lineNums));
        }
    ...
    private ControlFlowGraph buildCFG(FunctionBlock func) {
        ...
        for (int i = 0; i < func.bodyLines.size(); i++) {
            ...
            if (IF_KW.matcher(stripped).find()) {
                label = "If (line " + lineNum + ")";
                isDecision = true;
            } else if (ELIF_KW.matcher(stripped).find()) {
                label = "Elif (line " + lineNum + ")";
                isDecision = true;
            } else if (FOR_KW.matcher(stripped).find()) {
                label = "For (line " + lineNum + ")";
                isDecision = true;
            } else if (WHILE_KW.matcher(stripped).find()) {
                label = "While (line " + lineNum + ")";
                isDecision = true;
            } else if (EXCEPT_KW.matcher(stripped).find()) {
                label = "Except (line " + lineNum + ")";
                isDecision = true;
            } else {
                label = statementLabel(stripped, lineNum);
            }
    ...
    private static class FunctionBlock {
        ...
        final List<Integer> lineNums;

        FunctionBlock(String name, int startLine,
                      List<String> bodyLines, List<Integer> lineNums) {
            this.name = name;
